...
class CurvedEdges extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    path.quadraticBezierTo(firstCurve.dx, firstCurve.dy, flastCurve.dx, flastCurve.dy);
    
    final secondCurve = Offset(0, size.height - 20);
    final slastCurve = Offset(size.width-30, size.height - 20);
    path.quadraticBezierTo(secondCurve.dx, secondCurve.dy, slastCurve.dx, slastCurve.dy);
    
    final thirdCurve = Offset(size.width, size.height - 20);
    final tlastCurve = Offset(size.width, size.height);
    path.quadraticBezierTo(thirdCurve.dx, thirdCurve.dy, tlastCurve.dx, tlastCurve.dy);
